...
Graph.prototype.drawXAxis = function () {
	...
	//console.log(unit);
    while (xPos >= xPosIncrement) {
        if (this.drawTicks == true) {
            context.moveTo(xPos-2, this.centerY - this.tickSize / 2);
            context.lineTo(xPos-2, this.centerY + this.tickSize / 2);
            context.stroke();
        }
        if (this.drawNumbers)
    ...
    xPos = this.centerX + xPosIncrement;
    unit = this.unitsPerTickX;
    while (xPos <= this.canvas.width - xPosIncrement) {
        if (this.drawTicks == true) {
...
Graph.prototype.blinkDot = function (equation, x1, x2, times, direction, callback) {
    ...
    interval = setInterval(function () {
        ...
        if (times > 0) {
			...
			//console.log(equation(parseInt(x1)));
...
Graph.prototype.animateLine = function (equation, x1, x2,y1,y2, color, thickness, arrows, callBack) {
    ...
    time4 = window.setInterval(function () {
        ...
        //if ((x1 >= (x2) && diff > 0) || (x1 <= (x2) && diff < 0)) {
        if (increment >= 100 || (y1+d>=y2 && y1!=y2)) {
			...
			//if(x1!=x2)
			//{
				temp.drawLine(equation, x1, x1 + diff,y1,y1+d, color, thickness, '0|0');
				x1 += diff;
				d+=0.1;
				console.log(d);
				/*
...
Graph.prototype.drawLine = function (equation, x1, x2,y1,y2, color, thickness, arrows) {
    var context = this.context;
    arrows = arrows.split('|');
    var arrowTop = parseInt(arrows[0]);
    var arrowBottom = parseInt(arrows[1]);
    ...
    if (x1 != x2 || equation(x1) != equation(x2)) {
        context.beginPath();
       ...
       // console.log(equation(x1),equation(x2));
